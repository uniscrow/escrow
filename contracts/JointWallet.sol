...
contract JointWallet{
   ...
   struct Payout { 
    ...
    constructor(address _alice, address _bob, address erc20){
    ...
    where in a escrow the two parties under agreements are Alice and Bob
    ...
    */
    ...
    function initTransfer(uint256 qtyAlice, uint256 qtyBob) external{
        require(msg.sender == alice || msg.sender == bob , "not an owner");
        payouts.push (Payout(qtyAlice, qtyBob, msg.sender, address(0), State.pending));
