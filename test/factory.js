...
describe("WalletFactory", function () {
	...
	it("A new wallet with backup is created", async function () {
		await expect(this.factory.create2out3Backup(this.alice.address, this.bob.address, this.dave.address, this.backup.address, this.token.address)).to.emit(this.factory, "NewWallet");
		//.withArgs();
	});

	it("A new wallet with managing agent is created", async function () {
		await expect(this.factory.create2out3Managed(this.alice.address, this.bob.address, this.dave.address, this.agent.address, this.token.address)).to.emit(this.factory, "NewWallet");
		//.withArgs();
	});
});
